...
def generate_gpt_image(photo_jpg: bytes, depth_png: bytes, prompt: str) -> bytes:
    ...
    full_prompt = (
        ...
        "Image 1 is the real photo: white blocks placed on a surface. "
        ...
        "2. The white blocks are the ONLY buildings. For each white block, render one "
        "building on that exact footprint; use Image 2 for its height (brighter = "
        "taller, the raised top is the roof).\n"
        "3. Everywhere there is NO white block, render landscaping -- grass, trees, "
        "greenery, parks, walking paths. Everything that is not a white block is "
        "landscaped background.\n"
        "4. Keep the same number and positions of buildings as there are white "
        "blocks. Do not add, move, duplicate, or remove buildings.\n"
        "5. Output the same resolution and aspect ratio as Image 1.\n\n"
        "STYLE: " + prompt
    )
    fields = {
